...
\frame {
  ...
  \vspace{0.5cm}
  \underline{Не хвостовые} рекурсии \underline{не оптимизируются} трансляторами в \underline{итерационные алгоритмы}
...
\frame {
  ...
  \begin{itemize}
    \item убедиться, что функция вызывает себя небольшое количество раз (и уж точно не бесконечное число раз)
    \item если функция должна вызывать себя много раз --- убедиться, что рекурсия \underline{хвостовая}
  \end{itemize}
}
