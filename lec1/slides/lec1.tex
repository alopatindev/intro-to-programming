...
\begin{document}
  ...
  \section{О курсе}
    ...
    \frame {
      ...
      \begin{itemize}
        \item получим кругозор в программировании
  ...
  \section{Программировать просто?}
    \frame {
      \frametitle{Программировать просто?}
    }
    ...
    \frame {
      \frametitle{Зависит от}
      \begin{itemize}
        \item Специализации
        \item Интеллекта
        \item Способов обучения
      \end{itemize}
    }

    \frame {
      \frametitle{Проблема образования}
      \begin{itemize}
        \item курсы и книжки рассчитаны на студентов универов
        \item предполагается что студент имеет интеллект выше среднего
        \item большинство людей не такие $\Rightarrow$ программирование сложно для среднего человека (на самом деле нет)
      \end{itemize}
    }

    \frame {
      \sout{Программировать просто?}\linebreak

      Корректный вопрос: \underline{как} ты стал (хорошим) программистом?
    }

  \section{Вводные термины}
    \frame {
      \frametitle{Вводные термины}
    }
    ...
    \frame {
      ...
      \begin{center}
        Ввод $\rightarrow$ Обработка $\rightarrow$ Вывод
      \end{center}
    }
  ...
  \section{Обзор развития вычислительных систем}
    \frame {
      \frametitle{Обзор развития вычислительных систем}
    }
    ...
    \frame {
      ...
      \begin{center}
        \includegraphics[scale=0.06]{pictures/IBM_S360.jpg}
      \end{center}
...
%    }
    ...
    \frame {
      ...
      \vspace{0.5cm}
      \begin{center}
        \includegraphics[scale=0.25]{../../common/you_dont_say.png}
      \end{center}
    }
    ...
    \frame {
      ...
      \begin{center}
        \includegraphics[scale=0.6]{pictures/arm-arch.png}
      \end{center}
    }
    ...
    \frame {
      ...
      \begin{center}
        \includegraphics[scale=0.7]{../../common/pictures/programs-as-seen-by-users.png}
      \end{center}
    }
  ...
  \section{Парадигмы программирования}
    ...
    \begin{frame}
      ...
      \textbf{Описание желаемого результата}, \underline{вместо алгоритма} получения этого результата.\linebreak
        ...
        \vspace{0.5cm}
        \texttt{Выбрать имена абитуриентов}
...
%        \end{itemize}
        \begin{enumerate}
          \item Если небо светлое --- перейти к п. 2 иначе к п. 4
          \item Напечатать <<Сейчас день>>
          \item Перейти к п.5
          \item Напечатать <<Сейчас ночь>>
          \item Напечатать <<Ваш Капитан Очевидность>>
        \end{enumerate}
      }
  ...
  \section{Структурное программирование}
    ...
    \frame {
      ...
      \begin{itemize}
        ...
        \item использование двух \textbf{управляющих структур} (control structures)
        \begin{itemize}
          \item условие
          \item цикл
        \end{itemize}
    ...
    \frame {
      ...
      \begin{enumerate}
        \item Для \underline{i} от \underline{1} до \underline{n} с шагом \underline{1}
        \begin{enumerate}
          \item Выполнить действие
          \item Выполнить другое действие
          \item ...
    ...
    \frame[label=innerLoops] {
      ...
      \begin{enumerate}
        \item Для \underline{i} от \underline{1} до \underline{количество\_студентов} с шагом \underline{1}
        \item Напечатать имя i-го студента
        \begin{enumerate}
          \item Для \underline{j} от \underline{1} до \underline{количество\_оценок\_студента} с шагом \underline{1}
          \begin{enumerate}
            \item Показать j-ю оценку i-го студента
            \item Перейти к п. 2.1
          \end{enumerate}
          \item Перейти к п. 1
        \end{enumerate}
    ...
    \frame {
      ...
      Подсказки:
      \begin{itemize}
        \item дискриминант вычисляется по формуле $D = b^2 - 4ac$
        \item корни уравнения вычисляются по формуле $x_{1,2} = \frac{-b \pm \sqrt{D}}{2a}$
