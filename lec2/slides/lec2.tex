...
\begin{frame}[fragile]
  ...
  Например вместо
  \begin{minted}{python}
page = header + item * newsNumber + footer
  \end{minted}

\vspace{0.5cm}
написать что-то вроде
  \begin{minted}{python}
news = item * newsNumber
page = header + news + footer
  \end{minted}
\end{frame}
...
print x                 # [1, 2, 777, -55, 88]
  ...
  + --- оператор \textbf{конкатенации} (соединения) массивов
...
\end{frame}
    ...
    \frame {
      ...
      На \underline{любом} из трёх языков программирования: Python, JavaScript или Java (по +1 баллу за реализации на других языках, из перечисленных):
